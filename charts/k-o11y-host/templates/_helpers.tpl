...
app.kubernetes.io/component: {{ default "signoz" .Values.o11yHub.name }}
{{- end -}}
...
{{- if .Values.o11yHub.serviceAccount.create -}}
    {{ default (include "signoz.fullname" .) .Values.o11yHub.serviceAccount.name }}
{{- else -}}
    {{ default "default" .Values.o11yHub.serviceAccount.name }}
{{- end -}}
{{- end -}}
...
{{- $registryName := default .Values.o11yHub.initContainers.init.image.registry .Values.global.imageRegistry -}}
{{- $repositoryName := .Values.o11yHub.initContainers.init.image.repository -}}
{{- $tag := .Values.o11yHub.initContainers.init.image.tag | toString -}}
{{- if $registryName -}}
...
{{- $registryName := default .Values.o11yHub.initContainers.migration.image.registry .Values.global.imageRegistry -}}
{{- $repositoryName := .Values.o11yHub.initContainers.migration.image.repository -}}
{{- $tag := .Values.o11yHub.initContainers.migration.image.tag | toString -}}
{{- if $registryName -}}
    {{- printf "%s/%s:%s" $registryName $repositoryName $tag -}}
{{- else -}}
    {{- printf "%s:%s" $repositoryName $tag -}}
...
*/}}
{{- define "signoz.image" -}}
{{- $registryName := default .Values.o11yHub.image.registry .Values.global.imageRegistry -}}
{{- $repositoryName := .Values.o11yHub.image.repository -}}
{{- $tag := default .Chart.AppVersion .Values.o11yHub.image.tag | toString -}}
{{- if $registryName -}}
    {{- printf "%s/%s:%s" $registryName $repositoryName $tag -}}
{{- else -}}
    {{- printf "%s:%s" $repositoryName $tag -}}
...
*/}}
{{- define "signoz.port" -}}
{{- default 8080 .Values.o11yHub.service.port  -}}
{{- end -}}
...
{{- default 8085 .Values.o11yHub.service.internalPort  -}}
{{- end -}}
...
{{/*
Return the TLS secret name for otelCollector.
Uses existingSecretName if provided, otherwise generates from fullname.
*/}}
{{- define "otelCollector.tlsSecretName" -}}
{{- if .Values.otelCollector.tls.existingSecretName }}
{{- .Values.otelCollector.tls.existingSecretName }}
{{- else }}
{{- include "otelCollector.fullname" . }}-tls
{{- end }}
{{- end -}}
...
===== USER ENV VARIABLES =====
*/}}
{{- $userEnv := .Values.o11yHub.env | default dict -}}
...
{{- $legacyEnv = mergeOverwrite $legacyEnv .Values.o11yHub.additionalEnvs -}}
{{- end }}

{{- if and .Values.configVars .Values.o11yHub.configVars.clickHouseUrl }}
  {{- $legacyEnv = mergeOverwrite $legacyEnv (dict "signoz_telemetrystore_clickhouse_dsn" .Values.o11yHub.configVars.clickHouseUrl) -}}
{{- end }}

{{- $smtpSecretEnv := dict -}}
{{- if and .Values.o11yHub.smtpVars .Values.o11yHub.smtpVars.enabled .Values.o11yHub.smtpVars.existingSecret (not .Values.o11yHub.env.signoz_emailing_enabled) }}  {{- $smtpSecretEnv = merge $smtpSecretEnv (dict "signoz_emailing_enabled" .Values.o11yHub.smtpVars.enabled) -}}
  {{- with .Values.o11yHub.smtpVars.existingSecret }}
    {{- $secretName := .name -}}
    {{- if .fromKey }}
